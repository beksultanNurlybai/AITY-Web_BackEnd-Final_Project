function createLandmark() {
    ...
    .catch(error => {
        ...
        displayMessage('error', 'Failed to create blog post. Please try again later.');
    });
}
...
function updateLandmark() {
    ...
    .catch(error => {
        ...
        displayMessage('error', 'Failed to update landmark. Please try again later.');
    });
}
...
function deleteLandmark() {
    ...
    .catch(error => {
        ...
        displayMessage('error', 'Failed to delete landmark. Please try again later.');
    });
}
...
function fetchAllLandmarks() {
    ...
    .catch(error => {
        ...
        displayMessage('error', 'Failed to fetch all landmarks. Please try again later.');
    });
}
...
function displayLandmark(data, targetElementId) {
    ...
    data.pictures.slice(0, 3).forEach((picture, index) => {
        carouselHTML += `
            <div class="carousel-item ${index === 0 ? 'active' : ''}">
                <img src="${picture.substring(6)}" class="d-block w-100" alt="...">
            </div>
        `;
    });
    ...
    publishedDateElement.textContent = 'Posted on: ' + publishedDate.toLocaleString();
